...
NSString *const kTwitterConsumerKey = @"f3oDc4Wn42Ni2Zt4M5d9e2sO7";
NSString *const kTwitterConsumerSecret = @"SV1ikcgWlWu4GXXXv1eXJe67VfLNuAHiPhSMTM4fhjoixW9VFz";
...
- (void)loginWithCompletion:(void (^)(User *user, NSError *error))completion {
  ...
  [self fetchRequestTokenWithPath:@"oauth/request_token" method:@"GET" callbackURL:[NSURL URLWithString:@"cptwitterdemo://oauth"] scope:nil success:^(BDBOAuthToken *requestToken) {
    ...
    NSLog(@"%@", authURL);
    NSLog(@"Sent it to this URL: %@", [NSString stringWithFormat:@"https://api.twitter.com/oauth/authorize?oauth_token=%@", requestToken.token]);
...
- (void)openURL:(NSURL *)url {
  [self fetchAccessTokenWithPath:@"oauth/access_token" method:@"POST" requestToken:[BDBOAuthToken tokenWithQueryString:url.query] success:^(BDBOAuthToken *accessToken) {
    ...
    [self GET:@"1.1/account/verify_credentials.json" parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
      ...
      NSLog(@"current user, %@", user.profileImageUrl);
      self.loginCompletion(user, nil);
    } failure:^(AFHTTPRequestOperation *operation, NSError *error) {
      NSLog(@"failed getting current users.");
      self.loginCompletion(nil, error);
...
- (void)postNewStatus:(NSDictionary *)params {
  ...
  [self POST:@"1.1/statuses/update.json" parameters:params success:^(AFHTTPRequestOperation *operation, id responseObject) {
    NSLog(@"Neat, posted an update! %@", responseObject);
  } failure:^(AFHTTPRequestOperation *operation, NSError *error) {
    NSLog(@"Boo, it failed!, %@", error);
  }];
}
